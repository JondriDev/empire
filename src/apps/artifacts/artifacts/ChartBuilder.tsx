...
export default function ChartBuilder() {
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-br from-slate-950 via-cyan-950/20 to-slate-950 text-white overflow-hidden">
      <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between bg-black/20 backdrop-blur">
        <input
          value={title}
          onChange={e => setTitle(e.target.value)}
          className="bg-transparent text-2xl font-bold outline-none focus:bg-white/5 px-2 py-1 rounded"
        />
        <div className="flex items-center gap-2">
            ...
            {(['bar', 'line', 'pie'] as ChartType[]).map(t => {
              ...
              return (
                <button
                  key={t}
                  onClick={() => setType(t)}
                  className={`px-3 py-1.5 rounded-md text-sm flex items-center gap-1.5 transition ${type === t ? 'bg-indigo-500/30 text-indigo-200' : 'text-slate-400 hover:text-white'}`}
                >
                  <Icon size={14} /> {t}
                </button>
              )
          ...
          <button onClick={randomize} className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-sm flex items-center gap-2">
            <Shuffle size={14} /> Randomize
          </button>
          <button onClick={exportSVG} className="px-3 py-1.5 rounded-lg bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 text-sm flex items-center gap-2 hover:bg-emerald-500/30">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Data editor */}
        <div className="w-72 border-r border-white/10 bg-black/20 p-4 overflow-auto">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs uppercase tracking-wider text-slate-500 font-semibold">Data Points</h3>
            <button onClick={addRow} className="text-indigo-300 hover:text-indigo-200"><Plus size={14} /></button>
          </div>
            ...
            {data.map((d, i) => (
              <div key={i} className="flex items-center gap-2 group">
                ...
                <input
                  ...
                  className="bg-white/5 px-2 py-1 rounded text-sm flex-1 outline-none focus:bg-white/10"
                />
                <input
                  type="number"
                  value={d.value}
                  onChange={e => update(i, { value: Number(e.target.value) || 0 })}
                  className="bg-white/5 px-2 py-1 rounded text-sm w-16 outline-none focus:bg-white/10"
                />
                <button onClick={() => removeRow(i)} className="text-red-400/50 hover:text-red-400 opacity-0 group-hover:opacity-100 transition">
                  <Trash2 size={12} />
                </button>
              </div>
            ))}
          ...
          <div className="mt-6 pt-4 border-t border-white/10 text-xs text-slate-400 space-y-1">
            <div className="flex justify-between"><span>Total</span><span className="text-white font-mono">{total}</span></div>
            <div className="flex justify-between"><span>Max</span><span className="text-white font-mono">{max}</span></div>
            <div className="flex justify-between"><span>Avg</span><span className="text-white font-mono">{data.length ? Math.round(total / data.length) : 0}</span></div>
            <div className="flex justify-between"><span>Min</span><span className="text-white font-mono">{Math.min(...data.map(d => d.value))}</span></div>
          </div>
        </div>
              ...
              {data.map((d, i) => {
                ...
                return (
                  <g key={i}>
                    ...
                    <text x={x + w / 2} y={400} textAnchor="middle" className="fill-slate-400" style={{ fontSize: 12 }}>{d.label}</text>
                    <text x={x + w / 2} y={y - 6} textAnchor="middle" className="fill-white" style={{ fontSize: 11, fontWeight: 600 }}>{d.value}</text>
                  </g>
                )
              })}
            </svg>
              ...
              <svg id="chart-svg" viewBox="0 0 800 500" className="w-full h-full max-w-4xl">
                ...
                <g>
                  {data.map((d, i) => {
                    ...
                    return (
                      <text key={i} x={lx} y={ly} textAnchor="middle" className="fill-white" style={{ fontSize: 12, fontWeight: 700 }}>
