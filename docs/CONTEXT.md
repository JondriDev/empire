...
- **▶ EPIC-19 · The organism relates (the associative constellation) — ▶ ACTIVE.** 6th lens (ASSOCIATIVE): open any
  ...
  **Target: new `RELATED` QA guard 0 → 5/5.**
  ...
  - **✅ S2 DONE (this run):** `src/components/ui/RelatedConstellation.tsx` — subscribes `useGraph(s=>s.nodes)`, `useMemo(relatedTo(Object.values(nodes),nodeId))`; `null` when empty (self-hiding). Container `role="group"` + `[data-related=<nodeId>]`; ≤6 rows — **`<span role="button">` + `tabIndex`/`onKeyDown` (NOT a `<button>`)** because S3 nests it inside Timeline/Search row `<button>`s and button-in-button is invalid HTML (mirrors `NodeLineage`/`NodeDescendants`). Each row: app accent chip (`getAppIcon`/registry `color`, `color-mix` bg) + title + reason chip for `reasons[0]` → i18n `related.reason.{linked|sharedTag|sharedTerm|sameDay}` (EN/ID). `.related-*` CSS in `window-manager.css`. `data-related-item=<node.id>`, click/Enter → `openEntity`. Mounted in Network inspector per-entity list beside `<NodeLineage>` (`Network.tsx:710`). 4 render tests. **Note:** `src/components/ui/*` is EXEMPT from the control/keyboard audits (primitives dir) — the `<span role=button>` choice is about valid HTML nesting, not the metric.
  - **▶ S3 (next):** mount the SAME `<RelatedConstellation nodeId={n.id}/>` verbatim on Timeline (`src/apps/timeline/Timeline.tsx`) + Search (`src/apps/search/Search.tsx`) node rows, beside their existing `<NodeLineage>`. Import + mount only (no new component). Confirm the row still navigates (the span's `stopPropagation` keeps the outer row `<button>` from also firing) + is keyboard-operable. Extend `RelatedConstellation.test.tsx` (+2). **Acceptance:** build/vitest/eslint 🟢; six axes 0; mounted on both. **S4:** `RELATED` guard in `qa-smoke.mjs` on the headless-drivable Timeline → `5/5` (linkedTop·sharedTerm·sharedTag·unrelatedAbsent·oneTapLands) → EPIC-19 CODE-COMPLETE. **Trap:** seed `task`/`book`/`goal` types (note/learning/message get pruned by syncAll); drive Timeline, NOT the fragile Network canvas click.
...
## ✅ QA state (latest — 2026-07-16, EPIC-19 S2 shipped by Builder; full smoke pending QA)

- **S2 (Builder, this run):** `<RelatedConstellation>` built + mounted on the Network inspector. build🟢 vitest 638/638🟢 eslint🟢; all six axes 0 & LOCKED (`--assert-zero` exit 0). Auto-metrics: apps 31, test cases **531** (+4), files 72 (+1), bundle gz **735.4** (+1.2). `vitest run related` 21/21. **Not verifiable in cloud (no browser this run):** the component's rendered look + on-canvas navigation — describe-only; QA should re-smoke and eyeball the Network inspector constellation. Target `RELATED 0→5/5` NOT yet moved — EXPECTED (guard lands S4).
- **Prior full smoke (green `ae985...`/`ae505f3`, 2026-07-16):** all guards green — 32/32 routes, INBOUND 4/4, MEDIA 3/3, GRAPH-LEGIBLE 3/3, NODE-LINEAGE 1/1, TIMELINE 1/1, HOME-ATTENTION 6/6, SHELL-ATTENTION 4/4, PROV 3/3, PRECACHE 89 no-gap, OFFLINE 5/5. Env-only console noise (NOT bugs): maps tiles + weather geocoding + files/mail 401.
