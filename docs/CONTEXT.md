...
- **▶ ACTIVE: EPIC-14 · Shell conformance — the component shell becomes total (no app renders a bare interactive control).
  ...
  **Leap:** complete the `ui` set (`Select`/`IconButton`/`Segmented`) + migrate all 27 files onto it. **Target:** new
  ...
  - **▶ S5 (next — start here, no re-planning) — migrate the artifacts family = 27 → 0.** Files (heaviest-first): `src/apps/artifacts/artifacts/FormBuilder.tsx` (16; incl. a `<select>` → `Select`), `Flashcards.tsx` (9), `Kanban.tsx` (4), `ChartBuilder.tsx` (4), `MarkdownStudio.tsx` (1) — verify exact per-file counts via `node scripts/metrics.mjs | grep -A12 'Top off-shell'` (FormBuilder is now the #1 offender at 16). Apply the MIGRATION MAPPING RULE: text button→`Button`, icon-only→`IconButton` (`aria-label` from the action), **mutually-exclusive toggle/tab row → `Segmented`** (verify item `value`s are UNIQUE first — S3 colour-swatch trap; collide → `IconButton`+`aria-pressed`; a MULTI-select chip set → `Button`/`IconButton`+`aria-pressed`, NOT `Segmented` — see the Clock day-chips this run), `<select>`→`Select` (wrap in a fixed-width div if it sits in a row — see the Clock `w-40` note above), text `<input>`→`Input`, `<textarea>`→`TextArea`. Import `{ Button, IconButton, Segmented, Select, Input, TextArea }` from `../../components/ui`. Seam: `grep -nE '<button|<input|<select|<textarea' src/apps/artifacts/artifacts/*.tsx`. Add a `.test.tsx` per touched file locking the migrated a11y (Segmented → assert `getByRole('radio',{name})`+`aria-checked`, NOT `aria-pressed`). *Acceptance:* all five files drop off the offenders list, `offShellControls 284 → ~257`; build🟢 vitest🟢 eslint clean; `--assert-zero` exit 0; colour/style 0; bundle gz ±~0. **Render-confirm via `qa-smoke.mjs`: each artifact route renders clean.**
