...
- **▶ EPIC-19 · The organism relates (the associative constellation) — ▶ ACTIVE.** 6th lens (ASSOCIATIVE): open any
  ...
  **Target: new `RELATED` QA guard 0 → 5/5.**
  - **✅ S1 DONE (this run):** `src/lib/core/related.ts` — `RelatedReason`/`RelatedItem`, `significantTerms(node)` (title + `nodeBodyText` tokens, len≥4, minus STOP; imports `nodeBodyText` from `./search`), `relatedTo(nodes,id,limit=6)` (linked +8, shared-tag +4/tag cap 12, shared-term +3/term cap 9, same-day +2 via the canonical **local** `dayStamp` from `bridge.ts` — NOT `toISOString()` (UTC); drop 0; sort score↓→`meta.updated`↓; reasons order linked·shared-tag·shared-term·same-day). 17 unit tests. **Traps:** (a) a tag ≥4 chars scores BOTH shared-tag AND shared-term (significantTerms reads nodeBodyText, which folds in tags) — intentional; isolate a signal in tests via sub-4-char tags / distinct-day timestamps (default `created:0` = all same-day). (b) day buckets are the user's LOCAL calendar day (`dayStamp`, matches Calendar `data.date`) — a UTC `toISOString().slice(0,10)` bucket is a bug (fires on the wrong day off UTC-midnight); regression-locked with TZ-forced tests (Bug Hunter 2026-07-15).
...
## ✅ QA state (latest — 2026-07-16, green `ae505f3`, EPIC-19 S1 measure-only confirmed)

- All six axes 0 & LOCKED (`--assert-zero` exit 0). Auto-metrics: apps 31, test cases 524 (+10 vs `ea2aef2` — `messagesLogic.test.ts` + `LearningTracker.test.tsx` from two polish commits), files 70 (+2), bundle gz 734.2. build🟢 vitest🟢. **No runtime bug.** Full smoke on `ae505f3`: all guards green — 32/32 routes clean, INBOUND 4/4, MEDIA 3/3, GRAPH-LEGIBLE 3/3, NODE-LINEAGE 1/1, TIMELINE 1/1, HOME-ATTENTION 6/6, SHELL-ATTENTION 4/4, PROV-PERSIST/ENTITY 3/3, PRECACHE 89 no-gap, OFFLINE 5/5. **EPIC-19 S1 (pure `related.ts` engine) confirmed: `vitest run related` 17/17; target `RELATED 0→5/5` NOT yet moved — EXPECTED (guard lands S4; S1–S3 drive no headless guard, no contradiction).** Since `ea2aef2`, polish commits `7b594b9` (messages: scope sent to recipient) + `ae505f3` (learning-tracker: touch-reachable actions) landed — rebuilt + re-smoked, both render clean. Env-only console noise (NOT bugs): maps tiles + weather geocoding + files/mail 401 — all render clean.
