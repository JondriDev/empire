...
## ★ EPIC-14 S10 SHIPPED (2026-07-12, green main) — the Cakra family part 2 (agent + solver + settings) migrated onto the `ui` shell: `offShellControls 83 → 48 (−35)`, EXACTLY the S10 target. All six files 0: **AgentSurface 8→0, SolverPanel 8→0, SettingsPanel 7→0, ProblemDetail 6→0, ModelPicker 4→0, ConfirmModal 2→0** (`b72/i10/s0/t1 → b44/i4/s0/t0`). ▶ NEXT = **EPIC-14 S11** (shell components + artifacts wrappers, 48 → 0 — the LAST offenders → S12 LOCK).

**★★ THE SPACE-BETWEEN + `iconRight` RECIPE IS NOW BATTLE-TESTED FOR TOGGLE ROWS + SELECTABLE LIST ROWS (not just trailing chevrons).** This run it did the ModelPicker **Cakra-Auto toggle** (info block ‖ pill-switch) and every **model-list row** (name/ctx block ‖ `Check` when active) — `iconRight`=the trailing element, `children`=the leading block, `style={{justifyContent:'space-between'}}`. Same limit as always (the middle can't stretch-and-truncate), fine for these fixed-width modal rows. For the SolverPanel backlog rows + ProblemDetail SubTree rows I used the OTHER idiom — **leading-cluster left-aligned** (`icon`=glyph, `children`=info span, `style={{justifyContent:'flex-start', alignItems:'flex-start', whiteSpace:'normal'}}`) — because those want a top-aligned multi-line body, not a hard-right trailing element. **★ Border-on-a-ghost-Button trap:** ghost's variant style sets inline `border:'1px solid transparent'`; a Tailwind `border-b` class can't recolour it (inline wins). To get a bottom divider on a ghost Button, set `borderBottom:'1px solid var(--border)'` in the `style` prop (composes after the shorthand, overrides only the bottom edge) — do NOT set `borderColor` (that recolours all four sides into a full box).

**Per-file mapping applied (the MIGRATION MAPPING RULE):**
- **AgentSurface.tsx** (`b7/t1`) — model-picker pill → `Button ghost sm` (`iconRight=ChevronDown`, the tint bg + `radius-full` via `style`, inner spans wrapped in ONE gap-4 span to preserve inter-span spacing); the 4 header controls (thinking/panel toggles + clear + settings) → `IconButton` (`aria-pressed` on the two toggles, `aria-label`s added; the inline-svg trash → `Trash2` lucide); loading-cancel → `IconButton size="sm"` (absolute-positioned, kept). **★ TWO form-submit traps hit (both the known S7 patterns):** (1) send was `<button type="submit">` inside `<form onSubmit>` → converted `<form>`→`<div>`, send → `IconButton onClick={()=>handleSubmit()}` (`handleSubmit` already `(e?)=>e?.preventDefault()`; Enter-to-send lives on the textarea onKeyDown, unaffected). (2) the compose `<textarea>` had an auto-grow **`ref` callback** that fired every render — `TextArea` isn't a forwardRef so `ref` is a dead prop → gave it `id="cakra-agent-compose"` + a `useEffect([input])` that sizes it via `getElementById` (the AIChat S9 id-focus pattern; also covers inbound-clipboard prefill). `useActivityStore`/`runAgentTurn`/`runOrchestratedTurn`/`emit(APP_OPENED)` UNTOUCHED.
- **SolverPanel.tsx** (`b6/i2`) — daily-budget `<input type="number">` → `Input type="number"` (`value={String(dailyBudget)}`, `onChange={v=>setDailyBudget(Number(v))}`); auto-solver Start/Stop → `Button` (c-cakra tint via `style`, `icon` Play/Square); add-problem `<form onSubmit>` → `<div>` + `Input` (onKeyDown Enter→`submitNew`, made `submitNew(e?)`) + `IconButton` add; import-catalog → `Button ghost fullWidth` (border via style); category chips → `Button ghost sm` + `aria-pressed` (`radius-full` compact style); backlog rows → `Button ghost fullWidth` leading-cluster + `borderBottom` divider; world-feed Import → `Button ghost sm`. **`fetchWorldFeed()`/`feed.json` read path, `registerSolverIntents`, `startQueue`/`stopQueue`/`scoreOf` UNTOUCHED — solver renders clean + feed loads.**
- **SettingsPanel.tsx** (`b3/i4`) — close → `IconButton`; backend-URL `<input>` → `Input mono`; Save&test → `Button` (ion tint); per-provider apiKey `<input type=password>` → `Input` (type passthrough via rest); eye-toggle → `IconButton` (conditional aria-label); **temperature + maxTokens `<input type=range>` → `Slider`** (`onChange={v=>…}` — Slider emits NUMBER; the 256-step maxTokens gives ints, no parseInt). The two `<input type="checkbox">` (autoConfirm/showThinking) stay bare — **checkbox/radio/file are audit-EXEMPT** (no text-field primitive home), correct to leave.
- **ProblemDetail.tsx** (`b6`) — mobile back + ancestry breadcrumbs → `Button ghost sm` (breadcrumb keeps `hover:underline` class + compact style); runNext (disabled={busy}) + Plan→Tasks + Delete → `Button` (c-cakra tint / ghost-bordered); SubTree child rows → `Button ghost fullWidth` leading-cluster. `runQueueTick`/`nextStageFor`/`runIntent('solver-plan-to-tasks')` + the `window.confirm` delete guard UNTOUCHED.
- **ModelPicker.tsx** (`b4`) — close → `IconButton`; Cakra-Auto toggle + model-list rows → `Button ghost fullWidth` via the **space-between + iconRight recipe** (`aria-pressed`); provider tabs → `Button ghost sm` + `aria-pressed` (per-provider `p.color` via `style`). Chose Button-rows over `Segmented` for the model list **deliberately** — variable per-provider colours + a trailing Check + custom active tint don't fit the radiogroup wash.
- **ConfirmModal.tsx** (`b2`) — Cancel + Run-Anyway → `Button ghost` `className="flex-1"` (xenon-wash / ember-solid via `style` + `radius-lg`). Kept the two full-width side-by-side flex layout.

**Verify:** build🟢 vitest **532/532🟢** (Δ ±0 — no test files added; qa-smoke is the render gate) eslint clean (6 touched); `--assert-zero` exit 0 (token/util/style 0 Δ±0); bundle gz **730.9 (−0.2)**; no new deps. **Render-confirmed `node scripts/qa-smoke.mjs` (exit 0, auto-server): 32/32 routes render clean (solver + ai-chat uncaught:0), all 13 guards green** — INBOUND messages←ai-chat 4/4 ✅, PROVENANCE editor→ai-chat 3/3 ✅, GRAPH-LEGIBLE 3/3, INTENT-ROUNDTRIP 2/2, TIMELINE, HOME-ALIVE, PRECACHE 91 no-gap, OFFLINE 5/5. **Not visually PNG-inspected in-cloud** — the migrations are style-preserving by construction; on-device confirm the AgentSurface compose auto-grows + send-on-Enter, the SettingsPanel temp/maxTokens Sliders drag with the signal accent, the ModelPicker Cakra-Auto pill-switch + per-model Check sit hard-right, and the solver backlog/SubTree rows stay top-aligned + wrap.

**⚠️ QA 2026-07-12 (green main `07d8c43`) — S10 CONFIRMED HOLDING; `offShellControls` = 49 (+1 vs S10 target 48) reproduced.** First QA since S10 shipped: 32/32 routes render clean, all 14 guards green, `--assert-zero` exit 0, token/util/style all 0. S10's six files stay 0 (migration intact); the +1 is the NIM-commit bare `<button>` in `apps/cakra/AIChat.tsx:380` (independently caught by the Strategist in `7dd1e68`). Renders clean = DS regression only. bundle gz 730.9 → 732.5 (+1.6, NIM Cakra-Auto logic). Full detail in `docs/screenshots/latest/REPORT.md`.

**▶ NEXT = EPIC-14 S11 (shell + artifacts wrappers + one re-regressed Cakra file, 49 → 0 — the LAST offenders).** **★ Live census re-run by the Strategist 2026-07-12 (post-S10) = `offShellControls 49 (b45/i4/s0/t0)`, NOT 48 — the out-of-band Cakra NIM upgrade (`07d8c43`) re-introduced ONE bare control into the already-shipped S9 file `apps/cakra/AIChat.tsx` (`AIChat.tsx:380`, a new "Reasoning/Thinking" disclosure `<button onClick={()=>setOpen(o=>!o)}>` = Brain glyph + label + rotating ChevronRight). Migrate it FIRST with the space-between + `iconRight` recipe (`Button ghost fullWidth`, `icon={Brain}`, label as children, `iconRight={ChevronRight}` keeping the `rotate(90deg)` open-transform, `style={{justifyContent:'space-between', color:cssVar('signal')}}`) — it is a design-system regression that slipped in BECAUSE the metric isn't gated yet, the live proof of why S12 exists; S12's lock fails at 49≠0 if S11 misses it.** Rest of the census (heaviest-first; re-verify per-file with `node scripts/metrics.mjs` FIRST — counts shift as siblings migrate AND as later commits touch shipped files): `components/Desktop.tsx` (**8** `b7/i1`), `components/AppShell.tsx` (**6** `b6`), `apps/network/Network.tsx` (**4** `b4` — **keep the mesh/legend + inspector**), `apps/search/Search.tsx` (**4** `b3/i1` — **keep `GLOBAL-SEARCH` + `openEntity`**), `components/Bridge.tsx` (**4** `b3/i1` — **keep `HOME-ALIVE` widgets + the Cakra line**), `apps/artifacts/GeneratedSection.tsx` (**3**), `apps/artifacts/generated/ArtifactViewer.tsx` (**3**), `components/AppHost.tsx` (**3** `b3`), `components/ContextMenu.tsx` (**3** `b3`), `apps/artifacts/ArtifactsApp.tsx` (**2**), `apps/timeline/Timeline.tsx` (**2** `b2` — **keep `TIMELINE` facets**), `components/CommandPalette.tsx` (**2** `b1/i1` — **keep the ⌘K palette + `intentsFor`**), `components/Recents.tsx` (**2** `b2`), `apps/artifacts/ArtifactGallery.tsx` (**1**), `components/ErrorBoundary.tsx` (**1** `b1`). **★ DO NOT migrate `apps/artifacts/artifacts/ColorPalette.tsx` — it shows 13 bare controls in the raw census but is `DS_INFRA` (audit-EXEMPT in `metrics.mjs:52`), a colour-theory tool whose swatch buttons ARE the content; migrating it is pure churn for 0 metric gain.** *Acceptance:* `offShellControls = 0 (b0/i0/s0/t0)`; all 32 routes render clean + all 13 guards green; build🟢 vitest🟢 eslint clean; conformance 0. **After S11 lands 0, S12 = LOCK `offShellControls` in `--assert-zero` → EPIC-14 CODE-COMPLETE** (the lock is what stops exactly this NIM-style regression recurring). Full spec EPICS.md → EPIC-14 S11.
