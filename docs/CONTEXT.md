...
**▶ NEXT = EPIC-14 S12 · LOCK `offShellControls` in `--assert-zero` → EPIC-14 CODE-COMPLETE.** EXACT shape: (1) in `scripts/metrics.mjs` `--assert-zero` block (line ~293-303, beside the `tokenViolations`/`offSystemUtilities`/`offSystemStyle` gates), add `if (snapshot.offShellControls > 0) fail.push(\`offShellControls=${snapshot.offShellControls} (b${cv.dims.button}/i${cv.dims.input}/s${cv.dims.select}/t${cv.dims.textarea})\`)`, and add `offShellControls=0` to the success `console.log`. (2) Add a header-comment invariant to `src/components/ui/index.tsx` (top): *app code renders interactive controls through the `ui` primitives — a bare `<button>`/`<select>`/`<textarea>`/text-`<input>` in an app file fails CI.* (3) **VERIFY THE LOCK BITES:** temporarily add one bare `<button>` to any app file → `node scripts/metrics.mjs --assert-zero` exits 1 → REVERT. *Acceptance:* `--assert-zero` gates offShellControls=0 and goes RED on a single re-introduced bare control; build🟢 vitest🟢 eslint clean; conformance (all four axes) 0. This is a tiny, self-contained stage (metrics.mjs + a comment) — after it lands, EPIC-14 is CODE-COMPLETE (S1–S12) and the Strategist promotes the next epic. **The metric is currently 0 on main, so the lock will pass immediately; the ONLY real work is proving it BITES.**
