...
- **`docs/ARTISAN.md` now exists** (the Artisan's rotation ledger — seeded 2026-07-06 from `registry.ts`: 24 launcher apps + hidden Cakra-tab tools + The Bridge). Pick the least-recently-visited surface; a QA-flagged broken surface jumps the front. Visited: **`weather`** (2026-07-06), **`grammar`** (2026-07-09), **`language`** (2026-07-10), **`music`** (2026-07-10). ▶ NEXT = **`video`** (VideoPlayer — likely the SAME icon-only-transport a11y shape as Music, reuse the pass below).
