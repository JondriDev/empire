...
- **▶ EPIC-19 · The organism relates (the associative constellation) — ▶ ACTIVE.** 6th lens (ASSOCIATIVE): open any
  ...
  **Target: new `RELATED` QA guard 0 → 5/5.**
  - **✅ S1 DONE:** `src/lib/core/related.ts` — `relatedTo(nodes,id,limit=6)`/`significantTerms`/`RelatedReason`/`RelatedItem` (linked +8, shared-tag +4/tag cap 12, shared-term +3/term cap 9, same-day +2 via **local** `dayStamp` from `bridge.ts`, NOT UTC `toISOString`; drop 0; sort score↓→`meta.updated`↓; reasons order linked·shared-tag·shared-term·same-day). 17 tests. **Trap:** a tag ≥4 chars scores BOTH shared-tag AND shared-term (significantTerms reads `nodeBodyText`, which folds in tags) — isolate a signal in tests via sub-4-char tags / **distinct-day** timestamps (default `created:1` = same day).
  - **✅ S2 DONE:** `src/components/ui/RelatedConstellation.tsx` — subs `useGraph(s=>s.nodes)`, `useMemo(relatedTo(...))`; `null` when empty (self-hiding). Container `role="group"`+`[data-related=<nodeId>]`; ≤6 rows are **`<span role="button">`+`tabIndex`/`onKeyDown` (NOT `<button>`)** so the block stays valid HTML when nested inside a row `<button>` (mirrors `NodeLineage`). Each row: app accent chip (`getAppIcon`/registry `color`) + title + `reasons[0]` chip → i18n `related.reason.{linked|sharedTag|sharedTerm|sameDay}`. `.related-*` CSS in `window-manager.css`; `data-related-item=<node.id>`, click/Enter/Space → `openEntity`. (`src/components/ui/*` is EXEMPT from control/keyboard audits.)
  - **✅ S3 DONE (this run):** `<RelatedConstellation nodeId={n.id}/>` mounted verbatim on Timeline (`Timeline.tsx:294`, after `NodeDescendants`) + Search (`Search.tsx:305`, after `NodeLineage`) rows — import + mount only, no new component. Both sit INSIDE the row's ghost `<Button>` meta `<div>` (`flex flex-wrap`); the constellation is `width:100%` so it wraps onto its own line below the type/time/lineage. Valid nested because rows are `<span role=button>` (not `<button>`) + `stopPropagation` (verified by +2 tests: no-bubble-to-enclosing-row, Space-key parity → now 6 in `RelatedConstellation.test.tsx`, 651 total). build/vitest/eslint🟢; six axes 0 & LOCKED; bundle 735.7 (+0.1). **Trap hit:** eslint has NO `jsx-a11y` plugin — a `// eslint-disable jsx-a11y/*` comment ERRORS ("rule not found"); a bare `<div onClick>` in a TEST is fine (detector scans `src/` app code, tests exempt).
  - **▶ S4 (next) → ★ EPIC-19 CODE-COMPLETE:** add the `RELATED` guard to `scripts/qa-smoke.mjs` → `5/5` (linkedTop·sharedTerm·sharedTag·unrelatedAbsent·oneTapLands). Drive the **Timeline** surface (headless-drivable; the `[data-related=<id>]` group + `[data-related-item=<id>]` rows are the hooks), NOT the fragile Network canvas click. **Trap:** seed `task`/`book`/`goal` types (note/learning/message get PRUNED by `syncAll` in a bare QA seed); make two entities share a ≥4-char significant TERM (title token) for `sharedTerm`, a sub-4-char TAG for an isolated `sharedTag`, and an explicit `link` for `linkedTop`; a third with none for `unrelatedAbsent`. `oneTapLands` = click a `[data-related-item]` → its owning app window opens. Then move `RELATED 0→5/5` in metrics + retire EPIC-19 to DONE.
...
- **Sixth axis `docMass` (EPIC-16): 0 AND LOCKED in `--assert-zero` (S3; QA-reconfirmed green main `3f2d971` 2026-07-16 — 32/32 smoke clean, EPIC-19 S2 4/4 holds, RELATED guard S3/S4 still pending, no runtime bug).**
...
## ✅ QA state (latest — 2026-07-16, EPIC-19 S3 shipped by Builder; full smoke pending QA)

- **S3 (Builder, this run):** `<RelatedConstellation>` now mounted on ALL THREE surfaces — Network inspector (S2) + Timeline entity row + Search result row. build🟢 vitest 651/651🟢 eslint🟢; all six axes 0 & LOCKED (`--assert-zero` exit 0). Auto-metrics: apps 31, test cases **544** (+2), files 78, bundle gz **735.7** (+0.1). `vitest run related` 23/23. **Not verifiable in cloud (no browser this run):** the constellation's rendered look on the Timeline/Search rows — describe-only; QA should re-smoke and eyeball. The block wraps to its own line below each row's meta (type/time/lineage). Target `RELATED 0→5/5` NOT yet moved — EXPECTED (guard lands S4). **Last full smoke baseline (green, 2026-07-16):** 32/32 routes + all 14 guards green (INBOUND 4/4·MEDIA 3/3·GRAPH-LEGIBLE 3/3·NODE-LINEAGE 1/1·TIMELINE 1/1·HOME-ATTENTION 6/6·SHELL-ATTENTION 4/4·PROV 3/3·PRECACHE 89 no-gap·OFFLINE 5/5); env-only console noise (maps/weather/files+mail 401), NOT bugs.
