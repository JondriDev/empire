...
## ★ EPIC-14 S2+S3 QA RE-CONFIRMED (2026-07-11, green main `f0761ed`) — second independent QA, no app-code commit since the last (`f0761ed` is that QA commit). `metrics.mjs` reproduces the active-epic target EXACTLY again: `offShellControls = 307 (b243/i44/s6/t14)`, Δ ±0; `--assert-zero` exit 0; 32/32 routes clean; all 13 guards green; auto-metrics Δ ±0 (apps 31, tests 411, files 52, bundle gz 730.2). Visually re-inspected desktop/reader/calendar (shelled controls intact) + clock/photos/network — all clean, no error boundary. **No stage shipped → no movement expected; 307 holding is CORRECT.** No runtime bug, no drift. ▶ NEXT = **EPIC-14 S4** (Clock 11 + Photos 12 → 0).

## ★ EPIC-14 S2+S3 QA-CONFIRMED (2026-07-11, green main `2622813`) — first QA since both migrations shipped. `metrics.mjs` reproduces the active-epic target EXACTLY: `offShellControls = 307 (b243/i44/s6/t14)`, Δ ±0 vs the committed snapshot → **both S2 (Reader 341→322) and S3 (Calendar 322→307) acceptances CONFIRMED, net −34**. Reader + Calendar both dropped off the top-offenders list (now FormBuilder 16, Calculator 14, DataCenter 14, AIChat 13, Maps 12, Photos 12, Clock 11, Goals 10). `--assert-zero` exit 0 (colour/util/style all 0). 32/32 routes render clean (0 uncaught, 0 console err), all 13 guards green (INBOUND 4/4, MEDIA 3/3, GRAPH-LEGIBLE 3/3, GLOBAL-SEARCH 1/1, NODE-LINEAGE 1/1, INTENT-ROUNDTRIP 2/2, TIMELINE 1/1, HOME-ALIVE 1/1, PROVENANCE 3/3+3/3, PRECACHE 91 no-gap, OFFLINE 5/5). Auto-metrics Δ ±0 (apps 31, test cases 411, files 52, bundle gz 730.2). **Visually confirmed the two migrations render clean with their shelled controls:** `app-reader.png` (amber "Add book" + "+ Add your first book" primary Buttons, clean empty-state, no error boundary), `app-calendar.png` ("Today" pill Button + month chevron IconButtons + solid teal "+ Add Event" primary Button + Today's-Events sidebar, no error boundary). Env noise unchanged (mail 401 graceful, maps CARTO tiles blocked, weather geocode/geo blocked, files 401). No runtime bug, no drift. ▶ NEXT = **EPIC-14 S4** (Clock 13 + Photos 12 → 0).
...
- **▶ ACTIVE: EPIC-14 · Shell conformance — the component shell becomes total (no app renders a bare interactive control).
  ...
  **Leap:** complete the `ui` set (`Select`/`IconButton`/`Segmented`) + migrate all 27 files onto it. **Target:** new
  ...
  - **▶ S4 (next — start here, no re-planning) — migrate Clock (`src/apps/clock/Clock.tsx`, 13) + Photos (`src/apps/photos/Photos.tsx`, 12) = 25→0 — the `Segmented`/`IconButton`/`Select` showcase.** Apply the MIGRATION MAPPING RULE: text button→`Button`, icon-only→`IconButton` (`aria-label` from the action), **mutually-exclusive toggle/tab row → `Segmented`** (verify item `value`s are UNIQUE first — see the S3 colour-swatch trap; if they collide, fall back to `IconButton`+`aria-pressed`), `<select>`→`Select`, text `<input>`→`Input`, `<textarea>`→`TextArea`. **Clock:** tab row → `Segmented`, quick-set-minute chips → `Segmented`, the `addCityTz` `<select>` (~`:267`) → `Select`, alarm/remove/day-toggle icon buttons → `IconButton`. **Photos:** grid-size + grid/list + favorites-filter toggles → `Segmented`, tag chips → `Segmented`, lightbox nav/close/favorite/delete → `IconButton`. Behaviour-preserving: reuse each handler; **preserve Photos' media-store persistence** (do NOT touch the `mediaStore`/IDB path — `MEDIA-PERSISTS photos` axis must stay ✅). Seam: `grep -nE '<button|<input|<select|<textarea' src/apps/clock/Clock.tsx src/apps/photos/Photos.tsx`; import `{ Button, IconButton, Segmented, Select, Input }` from `../../components/ui`. *Acceptance:* both files drop off the offenders list, `offShellControls 307 → ~282`; build🟢 vitest🟢 eslint clean; `--assert-zero` exit 0; colour/style 0; bundle gz ±~0. **Render-confirm via `qa-smoke.mjs`: `/app/clock` + `/app/photos` render clean + the `MEDIA-PERSISTS photos` axis still ✅.**
  - **THEN S5–S8 migrate the remaining offender files heaviest-first** (artifacts
