...
- **▶ ACTIVE: EPIC-14 · Shell conformance — the component shell becomes total (no app renders a bare interactive control).
  ...
  **Leap:** complete the `ui` set (`Select`/`IconButton`/`Segmented`) + migrate all 27 files onto it. **Target:** new
  ...
  - **▶ S3 (next — start here, no re-planning) — migrate Calendar (`src/apps/calendar/Calendar.tsx`) 15→0, alone (complex date-grid).** Apply the MIGRATION MAPPING RULE: text button→`Button`, icon-only button→`IconButton` (`aria-label` from the action), **a row of mutually-exclusive view toggles (month/week/day, or any tab strip) → `Segmented`**, `<select>`→`Select`, text `<input>`→`Input`, `<textarea>`→`TextArea`. **Preserve the date-grid keyboard nav + the `useInboundHandoff` receive path + `<ProvenanceChip>` verbatim** (Calendar owns its events in its OWN localStorage and self-mirrors — do NOT touch that). Behaviour-preserving: reuse each handler. Seam: `grep -nE '<button|<input|<select|<textarea' src/apps/calendar/Calendar.tsx` to enumerate; import `{ Button, IconButton, Segmented, Select, Input }` from `../../components/ui`. *Acceptance:* Calendar drops out of the offenders list, `offShellControls 322 → ~307`; build🟢 vitest🟢 eslint clean; `--assert-zero` exit 0; colour/style 0; bundle gz ±~0. **Render-confirm via `qa-smoke.mjs` that `/app/calendar` renders clean + the `INBOUND-LANDS calendar/editor` axis still ✅ (Calendar is a handoff receiver — drive it).**
