...
| Metric | Current (QA 2026-07-02, after **EPIC-6 S3 · durable per-entity "From <source>" survives a reload** — green main `13a48dc`) | Target | Direction |
|---|---|---|---|
| Apps / routes | 26 | ~26 (steady) | coherence over new surface — not a growth metric |
| Test cases | 197 (static, +3 vs prior 194 = `LineageTrail.test.tsx`) · 239 (vitest run, +3 vs prior 236) | 60+ | ↑ higher = safer to leap |
| Test files | 25 (metrics, `src/` only, +1 `LineageTrail.test.tsx`) · 27 (vitest, incl. `scripts/precacheAudit.test.mjs` + `scripts/pwaBaseAudit.test.mjs`) | grow with code | ↑ |
...
| Bundle gz (KB) | 693.5 (≈flat, +1.0 vs 692.5 = the EPIC-6 S3 `LineageTrail` component + per-entity `from` plumbing, no new deps; the Reader's parsers are the mass, BY DESIGN; do NOT strip) | hold / shrink | ↓ |
...
| Metric | Source | Current (QA 2026-07-02, after **EPIC-6 S3 · durable per-entity "From <source>" survives a reload** green main `13a48dc`) | Target |
|---|---|---|---|
| Routes rendering clean | QA `REPORT.md` (headless render, no uncaught JS / blank) | **26 / 26** ✅ (27/27 incl. desktop shell; SHELL-IS-STYLED ✅ + REGISTRY-COVERAGE ✅ bidirectional, 26 apps; INBOUND-LANDS 3/3 ✅ + **MEDIA-PERSISTS 3/3 ✅ (music + video + photos)** + **PROVENANCE-PERSISTS 3/3 ✅ (durable app→app memory)** + **PROVENANCE-ENTITY 3/3 ✅ (S3 per-entity source survives reload)** + OFFLINE-BOOT 5/5 ✅ / PRECACHE 79 entries NO GAP) — **confirmed 2026-07-02 on green main `13a48dc`** (EPIC-6 S3 landed since last QA `3ef0955`): all 27 routes render with 0 uncaught JS, vitest 239/239, eslint 0, all guards green, `metrics.mjs --assert-zero` exit 0. Visually verified: Earth-from-Space launcher grid (desktop.png) + **the S3 headline surface** (`s3-lineage-goals.png` — durable `Goals ← Calculator` LineageTrail pill on the "Budget target 294" goal card AFTER reload, i.e. off the persisted entity, not the consumed sessionStorage chip) + **Maps** real Leaflet container w/ OSM/CARTO attribution (only tiles grey — env-blocked, net:8). No runtime regression. | 26 / 26 (every entity route) |
...
| **EPIC-6 target — durable app→app provenance persists across reload (`PROVENANCE-PERSISTS`) + per-entity source (`PROVENANCE-ENTITY`)** | QA `qa-smoke.mjs` guards + visual + code | **✅ S1 SPINE + S2 SURFACE + S3 PER-ENTITY ALL CONFIRMED LIVE 2026-07-02 (green main `13a48dc`).** `PROVENANCE-PERSISTS` 3/3 (edge ledger): 3 REAL `editor→<target>` handoffs from the Editor's ⚡ Send menu → recorded in `empire-provenance` by `startProvenanceTracking()`/`flowForEvent` → survive a full reload. **S3 (the HEADLINE-METRIC stage) NOW done-confirmed via the NEW `PROVENANCE-ENTITY` guard — 3 / 3:** each of `{calculator→goals, editor→messages, notes→calendar}` seeds an inbound payload → reloads to consume+prefill → triggers the app's OWN create/send so the entity persists a durable `from` → after a SECOND reload (sessionStorage chip gone) a `<LineageTrail>` (`role="note"`, aria-label `From <source>`) STILL renders off the persisted entity. Visually confirmed: `s3-lineage-goals.png` shows the durable `Goals ← Calculator` pill on the "Budget target 294" card after reload. S2 (Network Memory panel) remains done-confirmed (`network-memory{,-after-reload}.png`). **▶ NEXT = S4** (close the last graph-island: Reader's books → the mesh via `mirrorCollection('book','reader',…)` + `<NodeActions type='book'>`; with S1–S3 green, S4 CLOSES EPIC-6). | `PROVENANCE-PERSISTS` 3/3 + `PROVENANCE-ENTITY` 3/3 + Reader graph-legible |
