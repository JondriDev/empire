...
| Metric | Source | Current (QA 2026-07-03, on green main `436cebf` — **The Bridge · living home screen**, EPIC-9 S1 still ACTIVE) | Target |
|---|---|---|---|
| Routes rendering clean | QA `REPORT.md` (headless render, no uncaught JS / blank) | **27 / 27** ✅ (28/28 incl. desktop shell; SHELL-IS-STYLED ✅ + REGISTRY-COVERAGE ✅ bidirectional, 27 apps; INBOUND-LANDS 3/3 ✅ + **MEDIA-PERSISTS 3/3 ✅ (music + video + photos)** + **GRAPH-LEGIBLE 1/1 ✅** + **GLOBAL-SEARCH 1/1 ✅ (`tagOnly=true`)** + **NODE-LINEAGE 1/1 ✅ (EPIC-9 S1)** + **HOME-ALIVE 1/1 ✅ (NEW — The Bridge)** + **PROVENANCE-PERSISTS 3/3 ✅** + **PROVENANCE-ENTITY 3/3 ✅** + OFFLINE-BOOT 5/5 ✅ / PRECACHE 80 entries NO GAP) — **confirmed 2026-07-03 on green main `436cebf`** (The Bridge `436cebf` landed since last QA `f1303b6`): all 28 routes render with 0 uncaught JS, vitest **288/288**, eslint 0, all guards green, `metrics.mjs --assert-zero` exit 0. Visually verified: the new **Bridge** home screen (`desktop.png` — "Good afternoon" greeting, "Ask Cakra anything…" bar, live TODAY/OPEN TASKS/GOALS/ORGANISM stat cards, app-launcher grid) + all 27 app routes + **Maps** real Leaflet container (only tiles grey — env-blocked, net:8). No runtime regression; metrics reproduce the builder's Bridge snapshot exactly (apps 27, static tests 246, bundle 700.7, Δ ±0). | 26 / 26 (every entity route) |
