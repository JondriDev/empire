...
|---|---|---|---|
| Routes rendering clean | QA `REPORT.md` (headless render, no uncaught JS / blank) | **26 / 26** ✅ (27/27 incl. desktop shell; SHELL-IS-STYLED ✅ + REGISTRY-COVERAGE ✅ bidirectional, 26 apps; INBOUND-LANDS 3/3 ✅ + **MEDIA-PERSISTS 3/3 ✅ (music + video + photos)** + OFFLINE-BOOT 5/5 ✅ / PRECACHE 78 entries NO GAP) — **re-confirmed 2026-06-30 on green main `f9ec888`** (2 commits past `c51f79f`: `d866a7a` Files graph-mirror, `f9ec888` security harden + Calendar month fix): all 27 routes render with 0 uncaught JS, vitest 216/216. **Calendar month fix verified** — shows **June 2026**, the 30th highlighted on **Tuesday** (correct). **Maps** real Leaflet container w/ OSM/CARTO attribution (only tiles grey — env-blocked, net:8). ⚠️ `npx eslint .` reports 2 pre-existing `react-refresh/only-export-components` errors in `icons/index.tsx` (NOT CI-gated, NOT a runtime bug — see REPORT.md). | 26 / 26 (every entity route) |
