...
| Metric | Current (QA 2026-07-02, after **EPIC-6 S2 · The Network remembers (durable Fed-by/Feeds + Memory panel)** — green main `f5ab6be`) | Target | Direction |
|---|---|---|---|
| Apps / routes | 26 | ~26 (steady) | coherence over new surface — not a growth metric |
| Test cases | 194 (static, +6 vs prior 188) · 236 (vitest run, +6 `provenance.test.ts` `fedBy`/`feeds`/`recentEdges` vs prior 230) | 60+ | ↑ higher = safer to leap |
...
| Bundle gz (KB) | 692.5 (≈flat, +0.7 vs 691.8 = the EPIC-6 S2 UI + `fedBy`/`feeds`/`recentEdges` helpers, no new deps; the Reader's parsers are the mass, BY DESIGN; do NOT strip) | hold / shrink | ↓ |
...
| Metric | Source | Current (QA 2026-07-02, after **EPIC-6 S2 · The Network remembers** green main `f5ab6be`) | Target |
|---|---|---|---|
| Routes rendering clean | QA `REPORT.md` (headless render, no uncaught JS / blank) | **26 / 26** ✅ (27/27 incl. desktop shell; SHELL-IS-STYLED ✅ + REGISTRY-COVERAGE ✅ bidirectional, 26 apps; INBOUND-LANDS 3/3 ✅ + **MEDIA-PERSISTS 3/3 ✅ (music + video + photos)** + **PROVENANCE-PERSISTS 3/3 ✅ (durable app→app memory)** + OFFLINE-BOOT 5/5 ✅ / PRECACHE 78 entries NO GAP) — **confirmed 2026-07-02 on green main `f5ab6be`** (EPIC-6 S2 landed since last QA `312033c`): all 27 routes render with 0 uncaught JS, vitest 236/236, eslint 0, all guards green, `metrics.mjs --assert-zero` exit 0. Visually re-verified: Earth-from-Space launcher grid (desktop.png) + **The Network's NEW persistent Memory panel** (`network-memory.png` — 5 seeded durable edges rendered newest-first as `source → target` rows; reload keeps them while the Live ticker stays "awaiting signal…" — see `network-memory-after-reload.png`) + **Maps** real Leaflet container w/ OSM/CARTO attribution (only tiles grey — env-blocked, net:8). No runtime regression. | 26 / 26 (every entity route) |
...
| **EPIC-6 target — durable app→app provenance persists across reload (`PROVENANCE-PERSISTS`)** | QA `qa-smoke.mjs` guard + visual + code | **✅ S1 SPINE + S2 SURFACE CONFIRMED LIVE 2026-07-02 (green main `f5ab6be`) — 3 / 3.** `PROVENANCE-PERSISTS` guard fires 3 REAL `editor→<target>` handoffs from the Editor's ⚡ Send menu (`editor→notes` via NOTE_CREATED-from, `editor→ai-chat` + `editor→prompt-generator` via HANDOFF) → each edge is recorded in the durable `empire-provenance` store by `startProvenanceTracking()`/`flowForEvent`, then **survives a full reload** (rehydrated from the persisted ledger — jsdom cannot exercise this; `provenance.test.ts` pins only the pure fold/filter). **S1 (the memory spine) done-confirmed. S2 (Network *visibly* remembers) NOW done-confirmed too:** seeded 5 durable edges → the bottom-left **Memory panel renders them newest-first** as `source → target` rows and they **persist across reload** (age ticks `21s→24s` = same data re-read; Live ticker stays empty) — `S2-MEMORY-VISIBLE-PERSISTS ✅`, screenshots `network-memory{,-after-reload}.png`. Inspector Fed-by/Feeds helpers unit-pinned (+6 vitest); the inspector section needs a clicked satellite node → not captured headless. **▶ NEXT = S3** (durable per-entity "From <source>" survives reload — the HEADLINE-METRIC stage; Calendar/Goals/Messages still read `from` from sessionStorage). | `PROVENANCE-PERSISTS` 3/3 + Reader graph-legible |
