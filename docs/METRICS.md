...
| Metric | Current (QA 2026-07-03, after **EPIC-8 S2 · hits land on their exact entity + tags become searchable** — green main `1db665e`) | Target | Direction |
|---|---|---|---|
| Apps / routes | 27 (±0 — steady since S1 added **Search** at `/app/search`) | ~26 (steady) | coherence over new surface — not a growth metric |
| Test cases | 215 (static, +2 vs S1's 213 = `search.test.ts` array-flatten + tag-only cases) · 257 (vitest run, +2 vs 255) | 60+ | ↑ higher = safer to leap |
| Test files | 27 (metrics, `src/` only, ±0) · 29 (vitest, incl. `scripts/precacheAudit.test.mjs` + `scripts/pwaBaseAudit.test.mjs`) | grow with code | ↑ |
...
| Bundle gz (KB) | 696.4 (+0.4 vs S1's 696.0 = the EPIC-8 S2 array-flatten + `openEntity` + Notes focus-land, no new deps) | hold / shrink | ↓ |
...
| Metric | Source | Current (QA 2026-07-03, after **EPIC-8 S2 · hits land on their exact entity + tags become searchable** green main `1db665e`) | Target |
|---|---|---|---|
| Routes rendering clean | QA `REPORT.md` (headless render, no uncaught JS / blank) | **27 / 27** ✅ (28/28 incl. desktop shell; SHELL-IS-STYLED ✅ + REGISTRY-COVERAGE ✅ bidirectional, 27 apps; INBOUND-LANDS 3/3 ✅ + **MEDIA-PERSISTS 3/3 ✅ (music + video + photos)** + **GRAPH-LEGIBLE 1/1 ✅** + **GLOBAL-SEARCH 1/1 ✅ (now w/ `tagOnly=true` — S2 tag/array search)** + **PROVENANCE-PERSISTS 3/3 ✅** + **PROVENANCE-ENTITY 3/3 ✅** + OFFLINE-BOOT 5/5 ✅ / PRECACHE 80 entries NO GAP) — **confirmed 2026-07-03 on green main `1db665e`** (EPIC-8 S2 landed since last QA `ce30e4e`): all 28 routes render with 0 uncaught JS, vitest 257/257, eslint 0, all guards green, `metrics.mjs --assert-zero` exit 0. Visually verified: Earth-from-Space launcher grid (desktop.png) + all 27 app routes incl. **Search** (`app-search.png`, styled field + empty state) + **Maps** real Leaflet container (only tiles grey — env-blocked, net:8). No runtime regression. | 26 / 26 (every entity route) |
